  ...
  return (
    <div className='w-52 bg-zinc-100 p-4 rounded-md flex gap-4 pb-10 relative'>
      ...
      <div className=''>
        ...
        <button onClick={()=>handleClick(index)} className={`px-2 py-2 ${added === false ? "bg-orange-600" : "bg-teal-700"} absolute text-white text-sm bottom-0 rounded-full ml-12`}>{added === false ? "Add to favourite" : "Added"}</button>
    </div>
  )
}
